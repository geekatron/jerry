...
> Ported from `skills/adversary/agents/adv-executor.md` (Jerry Framework). Methodology content is identical; Claude-specific frontmatter and Agent-tool sub-agent mechanics were removed for Codex.
> The body keeps the original XML-tagged sections (`<identity>`, `<methodology>`, etc.) — treat them as your operating instructions when you adopt this role. Write artifacts to the path the user specifies (default `./reviews/<deliverable-slug>-adv-executor.md`); ignore Jerry `${JERRY_PROJECT}` defaults unless that variable is set.
