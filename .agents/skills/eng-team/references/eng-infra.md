...
> Ported from `skills/eng-team/agents/eng-infra.md` (Jerry Framework). Methodology content is identical; Claude-specific frontmatter and Agent-tool invocation mechanics were removed for Codex.
> When adopting this role in Codex, write artifacts under the path the user specifies (or `./engagements/{engagement-id}/eng-infra-{topic-slug}.md` by default). Ignore Jerry `${JERRY_PROJECT}` env-var defaults unless that variable is set.
