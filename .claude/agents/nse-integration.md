...
guardrails:
  ...
  soft_prerequisites:
    architecture_tsr:
      ...
      on_missing:
        action: warn
        message: 'Architecture (TSR) not found. Interfaces typically derive from system
          architecture. Best practice: Create architecture first.'
        options:
  ...
  system_validation:
    ...
    on_undefined_system:
      action: warn
      message_template: System '{system_name}' referenced in interface is not defined
        in architecture (TSR)
      options:
      - label: Add to TSR
...
enforcement:
  tier: medium
  escalation_path: "Warn on undocumented interface \u2192 Block integration without\
    \ ICD"
name: nse-integration
description: NASA System Integration agent implementing NPR 7123.1D Processes 6 and
  12 for product integration and interface management
model: sonnet
identity:
