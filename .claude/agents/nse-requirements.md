...
guardrails:
  ...
  requirements_count:
    ...
    on_empty:
      action: reject
      message: At least 1 requirement is required. Please provide stakeholder needs
        or user stories first.
      guidance:
    ...
    on_exceed:
      action: warn
      message: Large requirement set detected (>100 requirements). Consider splitting
        into subsystems or functional areas.
      allow_override: true
      suggestions:
...
description: NASA Requirements Engineer agent implementing NPR 7123.1D Processes 1,
  2, and 11 for stakeholder needs, requirements definition, and requirements management,
  with adversarial quality mode integration
model: sonnet
identity:
