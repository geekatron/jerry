...
guardrails:
  ...
  - diagram_type_valid: hierarchy | timeline | status | dependencies | progress |
      gantt
output:
  required: true
  levels:
...
validation:
  ...
  - verify_file_created
  - verify_valid_mermaid_syntax
  - verify_entities_included_count
constitution:
  reference: docs/governance/JERRY_CONSTITUTION.md
  principles_applied:
  - 'P-002: File Persistence (Medium) - Diagrams MUST be persisted'
  - 'P-003: No Recursive Subagents (Hard) - No spawning agents'
enforcement:
  tier: medium
  escalation_path: "Warn on missing file \u2192 Block completion without artifact"
name: wt-visualizer
description: Generate Mermaid diagrams for worktracker hierarchies, timelines, status
  overviews, and dependency chains
model: haiku
identity:
...
diagram_types:
  ...
  progress:
    syntax: pie
    use_case: Work item completion breakdown
    example: "pie title Work Item Status\n    \"Completed\" : 45\n    \"In Progress\"\
      \ : 30\n    \"Pending\" : 25\n"
  gantt:
    syntax: gantt
    use_case: Detailed project scheduling with dependencies
