...
Jerry is installed as a Claude plugin. The `jerry` entry point lives in the plugin
cache, **not** in the current working directory. You must pass `--project` so UV
resolves the correct virtual environment regardless of where you invoke the command.

```bash
# CORRECT - Use the --project flag to point at the installed plugin
uv run --project "~/.claude/plugins/cache/jerry-framework/jerry/0.21.0" jerry transcript parse <file.vtt> --output-dir <dir>
uv run --project "~/.claude/plugins/cache/jerry-framework/jerry/0.21.0" jerry session start
uv run --project "~/.claude/plugins/cache/jerry-framework/jerry/0.21.0" jerry items list

# WRONG - omitting --project fails when CWD is not the Jerry plugin root
uv run jerry transcript parse <file.vtt>   # ❌ FAILS outside plugin directory
uv run jerry session start                  # ❌ FAILS outside plugin directory

# WRONG - Don't use verbose module paths
uv run python -m src.interface.cli.main <args>  # ❌ VERBOSE, use jerry instead
```

> **Version pin:** Replace `0.21.0` with the version you have installed.
> List available versions with: `ls ~/.claude/plugins/cache/jerry-framework/jerry/`
...
| Task | Command |
|------|---------|
| Run CLI | `uv run --project "~/.claude/plugins/cache/jerry-framework/jerry/0.21.0" jerry <command>` |
| Run tests | `uv run pytest tests/` |
| Add dependency | `uv add <package>` |
| Add dev dependency | `uv add --dev <package>` |
| Sync dependencies | `uv sync` |
| Show installed | `uv pip list` |
| List installed versions | `ls ~/.claude/plugins/cache/jerry-framework/jerry/` |
...
*Updated: 2026-02-26 - BUG-001: Added --project flag requirement for plugin-installed jerry CLI*
