...
| H-29 | All file references in SKILL.md MUST use full repo-relative paths (`skills/saucer-boy/references/voice-guide.md`), not relative paths (`references/voice-guide.md`). Claude's working directory at load time is session-state-dependent, not skill-location-dependent. | Relative paths resolve against CWD (unpredictable), not the skill folder. Full repo-relative paths are the only style that resolves correctly regardless of session state. |
