...
### Security
- **BUG-010 (red-team remediation):** `jerry ast` temp-root containment hardened per RED-BUG010 red-vuln findings — (1) H-01 (CWE-552/CWE-668/CWE-281): a temp-default-root match (`tempfile.gettempdir()`/`/tmp`, not the project root or an explicit `--root`) is now gated on file ownership (`st_uid == os.geteuid()` on POSIX; no-op on Windows, whose per-user `%TEMP%` already isolates by user), closing a shared-host/CI multi-tenant read/write gap; (2) H-02/H-08 (incomplete-allowlist gap): `_is_broad_containment_root` now flags any ancestor of `$HOME` (not just the exact filesystem/drive root or exact `$HOME`), closing the `/home`, `/Users`, `$HOME`'s parent, and `C:\Users` broad-root warning coverage gaps
...
- **BUG-010 (follow-up)**: widen `jerry ast` containment defaults to include OS temp/scratchpad directories (`tempfile.gettempdir()`, `/tmp`) and add a `--root` exclusive-override flag on all 10 `jerry ast` subcommands, per PR #341 owner review — with a one-line stderr warning when `--root` resolves to an unusually broad location (filesystem/drive root or `$HOME`) and a one-line stderr transparency note when containment matches only via a temp/scratchpad default root (#337, #341)
