...
### Security
- **BUG-010 (Option C — user-declared trusted roots):** `jerry ast` path containment redesigned so the default allowed set is the user's project root **plus** explicitly-configured `ast.trusted_roots` entries (env `JERRY_AST__TRUSTED_ROOTS`, or `.jerry/config.toml`). No OS temp/scratchpad directory is auto-trusted. `--root` stays an exclusive per-invocation override; a new `--quiet` flag suppresses stderr advisory notes so stdout carries only the JSON/render payload. Broad roots (filesystem/drive root, `$HOME`, or an ancestor of `$HOME`) emit a stderr warning; a match via a configured non-project root emits a one-line stderr transparency note. A C4 adversarial tournament closed six Criticals of the earlier auto-widen approach (index-based trust, write-path TOCTOU [CWE-367], fail-open ownership gate, same-uid/uid-0 multi-tenant, `TMPDIR` poisoning, stderr/JSON corruption) plus three config-input-hygiene findings (an empty entry trusting cwd, a `JERRY_PROJECT` `..` traversal reading outside `projects/`, and silent relative entries). The interim temp-auto-trust and temp-root ownership-gate approach — flagged by the tournament and never released — was removed; `_is_broad_containment_root` ancestor-of-`$HOME` detection (cross-platform, covering `/home`, `/Users`, `C:\Users`) was retained. All `jerry ast` diagnostic/error messages now print to stderr, so stdout carries only the JSON/render payload for pipeline consumers (#337, #341, #371)
...
### Fixed
- **fix(ci):** security-scan CVE alert issue now lists the actual findings and a valid date (STORY-028 AC-4) — the `security-audit` composite action exposes a `vuln-details` output (pip-audit `--desc` table), `security-scan.yml` embeds it in a fenced block, and the scan date is computed in-shell via `date -u` instead of the non-existent `github.run_started_at` context property that rendered blank (#301)
...
### Added
- **docs(proj-032):** scaffold PROJ-032 nuclear-sop-review — dedicated project (plan, worktracker, ready-to-paste session prompt) for reviewing + remediating the contributed /nuclear-sop skill from PR #269: standards compliance, /eng-team review, full C4 adversarial tournament, merge recommendation
