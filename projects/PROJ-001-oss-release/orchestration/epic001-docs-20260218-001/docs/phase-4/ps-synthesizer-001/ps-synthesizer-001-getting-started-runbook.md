...
- [ ] `uv` is installed and on your PATH — confirm with `uv --version`
- [ ] Jerry is cloned to your local filesystem (recommended: `~/plugins/jerry`)
- [ ] The Jerry plugin (`jerry-framework`) is installed in Claude Code — confirm with `/plugin` > Installed tab

If any of these are not in place, complete [`../INSTALLATION.md`](../INSTALLATION.md) first, then return here.
...
Jerry requires an active project to operate. Every session, skill invocation, and output artifact is scoped to a project. This is enforced by rule **H-04**: the `JERRY_PROJECT` environment variable MUST be set before any Jerry workflow can proceed.
...
Depending on which agents ran, you may see output under subdirectories such as `research/`, `analysis/`, or `synthesis/`. The exact directory depends on which agents the skill invoked for your request.
...
Expected result: One or more `.md` files exist under `projects/PROJ-001-my-first-project/` that were not there before Step 4. Files named `PLAN.md` and `WORKTRACKER.md` are pre-existing from Step 1 — look for new files in subdirectories such as `docs/research/` or `docs/analysis/`.
