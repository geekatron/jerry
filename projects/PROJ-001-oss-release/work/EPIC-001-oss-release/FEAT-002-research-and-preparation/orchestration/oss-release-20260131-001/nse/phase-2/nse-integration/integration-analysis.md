...
- **House A (private repository):** The private residence with all amenities
...
| **Direction** | Unidirectional (internal → jerry) |
...
Invariant: jerry version ALWAYS <= internal repository version
Exception: External hotfix may be applied to jerry first
           (must be backported to internal repository)
```
...
| **Direction** | Filter (internal repository → sync-package) |
...
| **Type** | Manual Process |
| **Direction** | jerry → internal repository (exception flow) |
...
│     ▼                                                                  │
│  3. If ACCEPT:                                                         │
│     ├── Create corresponding branch in internal repository             │
│     ├── Cherry-pick or re-implement changes                            │
│     ├── Run internal CI/tests                                          │
│     ├── Merge to internal repository main                              │
...
├─────────────────────────────────────────────────────────────────────────┤
│                                                                         │
│  INPUT: internal repository (full)                                      │
│                                                                         │
│  PROCESS:                                                               │
│  ┌─────────────────────────────────────────────────────────────────┐   │
│  │  rsync -av --progress \                                          │   │
...
|----------|-------|-------------------|----------------|
| **Functional Baseline** | Internal repository architecture | Architecture team | ADR required |
...
│  INVARIANTS:                                                            │
│  ───────────                                                            │
│  1. jerry:tag ALWAYS corresponds to internal:tag (same version)          │
│  2. jerry:main ALWAYS equals or lags internal:main (never ahead)        │
...
|----------|----------|---------|----------------|
| .sync-config.yaml | Internal repository root | Sync configuration | Path changes |
...
*This document was produced by nse-integration agent as part of Phase 2 Tier 3 for PROJ-001-oss-release.*
