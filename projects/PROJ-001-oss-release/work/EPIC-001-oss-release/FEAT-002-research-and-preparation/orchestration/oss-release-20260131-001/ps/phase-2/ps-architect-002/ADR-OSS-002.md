...
- **internal repository** (private): Contains proprietary extensions, internal projects, and development work
...
│  │  │ Format: "sync: Sync from internal vX.Y.Z [YYYY-MM-DD]"      │  │ │
│  │  └────────────────────────────────────────────────────────────┘  │ │
│  │                                                                   │ │
│  └───────────────────────────────────────────────────────────────────┘ │
│                              │                                          │
...
│  ├───────────────────────────────────────────────────────────────────┤ │
│  │                                                                   │ │
│  │  Synced content from internal repo (no projects/, internal/, etc.) │ │
│  │                                                                   │ │
│  └───────────────────────────────────────────────────────────────────┘ │
│                                                                         │
└─────────────────────────────────────────────────────────────────────────┘
...
#### `.sync-config.yaml` (in internal repository root)

```yaml
# Jerry Repository Sync Configuration
# This file defines what content syncs from internal repository to jerry (public)
...
commit_template: |
  sync: Sync from internal v{version} [{date}]
    ...
    steps:
      ...
      - name: Commit and push
        ...
        run: |
          ...
          COMMIT_MSG="sync: Sync from internal ${{ github.event.inputs.version }} [$(date -u +%Y-%m-%d)]

          Synced from internal commit: ${{ needs.prepare-sync.outputs.sync_sha }}
...
│     ├─── ACCEPT ───────────────────────────────────────────────┐    │
│     │                                                          │    │
│     │  3. Create corresponding branch in internal repo          │    │
│     │     │                                                    │    │
│     │     ▼                                                    │    │
...
|---|------|--------|-------|--------------|
| 1 | Create `.sync-config.yaml` in internal repository | 30 min | DevOps | File exists with valid YAML |
...
| 5 | Generate `SYNC_PAT` with repo scope | 15 min | Admin | Secret added to internal repository |
        ...
        with:
          script: |
            github.rest.issues.create({
              ...
              body: 'The drift detection workflow found differences between the internal repository and jerry (public). Please review and sync.',
...
*This ADR was produced by ps-architect-002 for PROJ-001-oss-release Phase 2.*
*Constitutional Compliance: P-001 (Truth), P-002 (Persistence), P-004 (Provenance), P-022 (Honesty)*
