...
The Jerry Framework has matured through PROJ-001 through PROJ-001 and is ready for open-source release. Phase 0 and Phase 1 analysis identified 22 risks, with Phase 2 producing 6 ADRs to address them. This ADR synthesizes all decisions into an executable checklist.
...
**L2 (Architect)**:
- **Unidirectional Flow**: internal → jerry ONLY (never reverse)
...
gh workflow run sync-to-public.yml \
  --repo org/jerry-internal \
  -f version=v0.1.0

# Monitor execution
gh run watch --repo org/jerry-internal

# Verify result
gh run view --repo org/jerry-internal --log
```
...
grep -r "internal-repo" . --include="*.md" --include="*.py"
