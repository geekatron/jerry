# TASK-006: Optimize error-handling-standards.md and tool-configuration.md
...
Optimize `.context/rules/error-handling-standards.md` and `.context/rules/tool-configuration.md` by applying rule IDs from TASK-003, replacing verbose explanations with concise HARD/MEDIUM/SOFT enforcement language, removing cross-file redundancy, replacing inline constants with references to EN-701 SSOT, and tagging critical rules for L2 re-injection.

### Acceptance Criteria

- [ ] `error-handling-standards.md` optimized with rule IDs and tier vocabulary
- [ ] `tool-configuration.md` optimized with rule IDs and tier vocabulary
...
- Can run in parallel with: TASK-004, TASK-005, TASK-007, TASK-008, TASK-009
...
|-------------|------|------|
| Optimized `error-handling-standards.md` | Rule file | -- |
| Optimized `tool-configuration.md` | Rule file | -- |
...
| Date | Status | Notes |
|------|--------|-------|
| 2026-02-14 | Created | Initial creation. Parallelizable with TASK-004, TASK-005, TASK-007 through TASK-009 after TASK-003 completes. |
