...
def validate_playbook(playbook_path: Path) -> PlaybookValidation:
    ...
    for lang, block, line_num in yaml_blocks:
...
def generate_report(validations: list[PlaybookValidation]) -> str:
    ...
    lines = [
        ...
        "",
        f"> **Generated:** {now}",
        f"> **Test ID:** WI-SAO-037",
        f"> **Initiative:** SAO-INIT-007 (Triple-Lens Playbook Refactoring)",
        "",
        "---",
        "",
        "## Summary",
    ...
    lines.extend([
        ...
        f"*Part of WI-SAO-037: Validate all examples executable*",
    ])
