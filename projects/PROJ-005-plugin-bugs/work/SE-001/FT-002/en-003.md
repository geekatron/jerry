...
|--------|-------------|--------|--------|
| [BUG-007](./bug-007.md) | session_start.py requires pip installation | FIXED | Full root cause analysis |
...
| [DISC-005](./disc-005.md) | PYTHONPATH required for uv run to access local src/ imports |
...
// After (includes PYTHONPATH per DISC-005)
...
**Acceptance Criteria:**
- [x] hooks.json updated with uv run command
- [x] Hook invokes src/interface/cli/session_start.py directly
...
1. Added to `src/interface/cli/session_start.py` (after docstring):
```python
# /// script
...
2. Deleted `scripts/session_start.py`

3. Updated `docs/INSTALLATION.md` with uv requirement section

**Evidence:**
- File: `src/interface/cli/session_start.py` lines 36-39
- File: `docs/INSTALLATION.md` (new "uv Requirements" section)
- Commit: (pending)

**Acceptance Criteria:**
- [x] PEP 723 metadata added to session_start.py
- [x] scripts/session_start.py deleted
- [x] INSTALLATION.md documents uv requirement
- [ ] Hook works on fresh clone without pip install (verification pending)
- [ ] All contract tests pass (verification pending)

---

## Verification Checklist

- [x] `PYTHONPATH="." uv run src/interface/cli/session_start.py` works from plugin root
- [ ] Output matches contract test expectations (13 tests) - pending
- [x] Works with JERRY_PROJECT set (project-context tag)
- [x] Works without JERRY_PROJECT (project-required tag)
- [x] Works with invalid JERRY_PROJECT (project-error tag)
- [x] Exit code is always 0
- [ ] Works on fresh clone without pip install - requires end-to-end test
...
| Source | Location |
|--------|----------|
| BUG-007 Report | [bug-007.md](./bug-007.md) |
