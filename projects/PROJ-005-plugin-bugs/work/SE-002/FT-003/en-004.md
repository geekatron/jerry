...
- [ ] Tests updated to use `src/interface/cli/session_start.py`
- [ ] Tests updated to execute via `uv run`
- [ ] Tests set PYTHONPATH for local imports
- [ ] Tests pass with new execution model
- [ ] Tests validate PEP 723 inline metadata presence (NEW)
...
|---|------|--------|----------|
| 1 | Verify existing tests are broken (run pytest) | PENDING | |
| 2 | Update `session_start_script` fixture to new location | PENDING | |
| 3 | Update `run_session_start` helper to use `uv run` | PENDING | |
| 4 | Add PYTHONPATH to subprocess environment | PENDING | |
| 5 | Run tests and verify they pass | PENDING | |
| 6 | Add test for PEP 723 metadata presence | PENDING | |
| 7 | Add test for standalone execution without pip | PENDING | |
| 8 | Verify all 23+ tests pass | PENDING | |
| 9 | Commit fix with evidence | PENDING | |
