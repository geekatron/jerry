...
Jerry faces this exact challenge. Per DEC-002 (Dual Repository Strategy), we need to split from one repository (`source-repository`, internal) to two repositories:
- **source-repository** (internal): Contains proprietary extensions, internal projects, development work
...
├── 0900: Run filtered copy from source-repository
├── 1000: Gitleaks scan
...
PHASE 4: CUTOVER (Day 5)
├── Final sync from source-repository
...
**Description:** Fork source-repository publicly, then remove internal content.

**Implementation:**
```
1. Fork source-repository to jerry
...
rsync -av --exclude-from=.sync-exclude source-repository/ jerry/
...
#### EXCLUDED from Public Repository (stays in source-repository)
...
**Decision:** The public repository will have **no git history** from source-repository.
...
│   3. Address issues in source-repository                                        │
...
| 1.4 | 1 | Generate and store SYNC_PAT | 15 min | Admin | Secret in source-repository |
...
│  │  (internal)        Sync at each release          (public)         │   │
│  │                                                                   │   │
...
*This ADR was produced by ps-architect-005 for PROJ-009-oss-release Phase 2.*
*Constitutional Compliance: P-001 (Truth), P-002 (Persistence), P-004 (Provenance), P-011 (Evidence)*
