...
1. **Leveraging ADR-007** (existing internal specification) as the authoritative template source
...
├── docs/
│   ├── PLAYBOOK.md            ← Step-by-step guide (existing)
│   ├── RUNBOOK.md             ← Operational procedures (existing)
│   ├── OUTPUT-GUIDE.md        ← NEW: OSS user-focused template guide
│   └── TEMPLATE-CONTRACTS.md  ← NEW: Model-agnostic guarantees
├── validation/
│   └── ts-critic-extension.md ← Validation criteria (existing)
└── schemas/
...
*This ADR was produced by ps-architect-006 for PROJ-009-oss-release Phase 2.*
