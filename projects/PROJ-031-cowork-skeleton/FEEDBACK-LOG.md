...
### FU.12 adversary-protocol-adr (user label: FU.0, 2026-07-07)

**Verbatim:**
> FU.0. One process note worth keeping: across both threads, the verified-criticals protocol proved itself in both directions — it refuted every claim against the mature ADR package (protocol artifact confirmed) and it unanimously confirmed a real regression in the FU-log package that a fix had introduced (real defect caught). That's the behavior you'd want from a permanent addition to Jerry's /adversary tournament methodology, and it's a candidate improvement to fold into the skill once these two packages ship — I'd suggest logging it as a follow-up work item when we do the GH-issue parity pass (#18).
> AN:
> Let's make an ADR for how to improve the `/adversary` tournament methodology, including design diagrams so that we can then review and create work items and GH-issue to enhance the skill and process.

**Summary:** User commissions an ADR for improving the `/adversary` tournament methodology — codifying the verified-criticals protocol (3-lens refutation panels), subtraction-first remediation doctrine, convergence-based stop conditions, and dual-protocol score transparency — WITH design diagrams (per FU.10 lesson), for user review → work items + GH issues to enhance the skill.

**Disposition:** **IN-PROGRESS** — ADR authored (`decisions/ADR-adversary-tournament-protocol-001-verified-criticals-methodology.md`; first born-Scheme-B ADR; 6 decisions D-1..D-6 from 17 options; 4 mmdc-validated diagrams; 8 work items + 6 draft GH issues, all PROPOSED). Review iterations 1-2 scored 0.66/0.65 with **6 panel-verified Criticals (unanimous)** — the protocol catching its own advocates: CV-001-20260707 exposed a misattribution of the fabricated-Glob anecdote that originated in the orchestrator's own turn summaries (finder-caught, panel-confirmed — not panel-originated; FU.1 disposition corrected in place); CC-001 caught the adv-verifier T1-vs-Write spec contradiction; CV-002 caught Figure 3 letting a verified Critical bypass remediation. Workflow extended to iteration-3 (within C3 ceiling 7): owner fix of the 6 → one more verified round → hand to user at gate or with honest status. Then: user review → work items + GH issues (#18).
