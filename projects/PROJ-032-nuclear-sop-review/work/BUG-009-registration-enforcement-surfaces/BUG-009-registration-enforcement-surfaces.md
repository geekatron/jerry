...
- [ ] H-22 rule cell in `.context/rules/mandatory-skill-usage.md` contains the /nuclear-sop sentence (exact wording from registration-trigger-map-row.md), inserted after the /contract-design clause.
- [ ] The L2-REINJECT comment enumeration in the same file includes /nuclear-sop.
- [ ] The /nuclear-sop trigger row's Compound Triggers extended with `"nuclear workflow" OR "nuclear sop" (phrase match)`; the /orchestration row untouched; the phase-6 collision-analysis artifact annotated as corrected/superseded.
- [ ] AGENTS.md updated: "Nuclear SOP Skill Agents" nav-table entry with anchor link; Agent Summary row (Nuclear SOP | 4 agents); Total 89 → 93; "Last verified" updated to the fix date; sop-* appended to the MCP "Not included (by design)" note with file-based-persistence-per-P-002 wording mirroring wt-*/eng-*/red-*.
- [ ] Validation passes: grep confirms "nuclear-sop" in the H-22 sentence, L2-REINJECT comment, and trigger row; simulated routing of "nuclear workflow" resolves to /nuclear-sop via compound trigger; AGENTS.md total equals the count of registered agents; every `##` heading in AGENTS.md appears in its nav table.
