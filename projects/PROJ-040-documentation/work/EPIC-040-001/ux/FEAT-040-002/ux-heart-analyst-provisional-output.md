...
1. **Getting-Started Completion Rate** (Task Success) — proportion of users who reach first-skill invocation within the 15-minute target window. This is the single highest-impact metric because the B=MAP analysis (FEAT-040-006) identifies a Major bottleneck (Prompt + Ability) in the getting-started flow at Step 3. It is directly actionable and corresponds to the highest-severity heuristic findings (F-014 Sev 3 — sidebar navigation gaps; F-016 Sev 2 — prerequisites not surfaced before Quick Start, FEAT-040-004 rescope-iter-2).

2. **Skill Catalog Discovery Rate** (Adoption) — proportion of new users who interact with documentation referencing more than the 6-7 skills listed in README/index.md. This metric directly measures the impact of F-020 (Severity 2, 7 of 19+ documented skills visible in Available Skills table, FEAT-040-004 rescope-iter-2). It is the most important leading indicator for adoption because undiscovered skills are unused skills. [Note: prior citation F-001 "stale skills table Sev 3" was INVALIDATED by live-site rescope — the live skills table is current; F-020 is the current live-site finding.]

3. **Documentation Satisfaction Score — SUPR-Q** (Happiness) — user-rated satisfaction with documentation quality. The Heuristic Evaluation (F-011, Severity 3 — jargon density without inline glossary, FEAT-040-004 rescope-iter-2) and B=MAP (FM-001 Motivation borderline at Belonging=3) both converge on a trust and confidence gap. A post-session SUPR-Q survey quantifies this gap and tracks improvement.
...
|-----------|----------|-----------|
| **Happiness** | Yes | Convergent evidence from three upstream analyses: F-011 (Severity 3 — jargon density without inline glossary or plain-language framing damages trust, FEAT-040-004 rescope-iter-2), FM-001 (Motivation borderline — Belonging=3, Social=3 using min-operator, FEAT-040-006), and B=MAP finding that "marketing tone creates instruction reliability uncertainty" (F-011 also covers H2 marketing-voice problem in live-site evaluation). User-facing documentation for a developer tool depends on perceived trust and confidence. Happiness is a leading indicator of retention. [Note: prior citations F-007 and F-003 from degraded-mode FEAT-040-004 iter-4 are superseded; F-011 is the current live-site equivalent.] |
| **Engagement** | Yes | F-020 (Severity 2, FEAT-040-004 rescope-iter-2) documents that 7 of 19+ documented skills are visible in the Available Skills table — the majority of the skill catalog is undiscoverable at entry points. The diataxis audit confirms zero tutorial coverage and 13% partial how-to coverage across all 30 skills. Engagement depth (are users exploring beyond the visible skills?) is directly measurable and directly tied to documentation remediation impact. [Note: prior citation F-001 "Severity 3, stale skills table" was INVALIDATED by live-site rescope; the live skills table is current. F-020 is the current live-site equivalent at Severity 2.] |
...
| **Retention** | Yes | Selected with LOW-MEDIUM confidence. No direct behavioral evidence for retention in the current analysis. However, F-013 (Severity 3, FEAT-040-004 rescope-iter-2 — skill-to-playbook linkage missing; skills table lacks hyperlinks to guides) creates a structural barrier to return visits: users who complete the getting-started flow cannot navigate to how-to content for skills beyond the entry-point table. Selected because the documentation gaps identified predict poor retention; this dimension requires instrumentation to confirm. [Note: prior citation F-004b "23 of 30 skills missing from Guides section" is superseded; F-013 is the current live-site equivalent.] |
| **Task Success** | Yes | Highest direct evidence. B=MAP analysis (FEAT-040-006) identifies specific failure modes in the getting-started flow: missing Facilitator prompt at Step 3, Brain Cycles=2 (below threshold). Heuristic findings F-014 (Severity 3 — sidebar navigation lacks breadcrumbs and recognition affordances) and F-016 (Severity 2 — prerequisites checklist not surfaced before Quick Start) map directly to Task Success. WCAG finding W-001 (SC 1.3.1, Severity 3, FEAT-040-005 rescope-iter-5 — INSTALLATION.md bold step labels not AT-navigable as headings) creates AT-specific task success barriers. Task Success is the primary measurement dimension for any documentation UX initiative. [Note: prior citations F-010 "hidden branching Severity 3" is superseded by F-014/F-016 in the live-site rescope; W-002 "non-descriptive link text 'file it'" was REMOVED as a false positive in FEAT-040-005 rescope-iter-2 — "file it" is plain prose, not a hyperlink — and is not cited here.] |
...
| **GSM Derivation** | F-011 (H2, Severity 3, FEAT-040-004 rescope-iter-2): jargon density without inline glossary or plain-language framing ("Context Rot," "HARD rules," "5-layer enforcement," "dialectical synthesis") degrades trust on first impression. F-011 also encompasses the marketing-voice problem in the live-site evaluation — the live site's feature table uses implementation language rather than user-benefit language, creating instruction reliability uncertainty. B=MAP FM-001 (FEAT-040-006): Motivation Belonging=3 and Social=3 (min-operator, borderline) indicate social proof weakness. All three converge: user confidence in the documentation is impaired. [Note: prior citations F-007 "inconsistent terminology, H4, Sev 3" and F-003 "marketing voice, H2, Sev 2" from degraded-mode FEAT-040-004 iter-4 are superseded; F-011 is the current live-site equivalent covering both dimensions.] |
...
| Component | Content |
|-----------|---------|
| **Goal** | Engagement Goal: Documentation users actively discover and explore beyond the initial entry-point content, interacting with skill-specific documentation at a breadth proportional to the full catalog — measured as the fraction of registered skills whose documentation users access, with a target that scales with the catalog size rather than a fixed absolute count. (Post-remediation framing: as F-020 remediation proceeds and the Available Skills table is expanded, the count of entry-point-visible skills will increase; this goal is defined as (distinct skill documentation pages visited / total registered skills) approaching an acceptable engagement depth, independent of how many skills are currently visible at entry points.) |
| **GSM Derivation** | F-020 (H1+H6, Severity 2, FEAT-040-004 rescope-iter-2): Available Skills table on the live site displays 7 of 19+ documented skills — the majority of the skill catalog is undiscoverable at entry points. F-013 (H10, Severity 3, FEAT-040-004 rescope-iter-2): skills table lacks hyperlinks to playbooks or guides — users see `/problem-solving` listed but cannot navigate to its documentation. Diataxis audit: zero tutorial coverage, 13% partial how-to coverage. If skills are invisible or unlinkable, engagement with their documentation is impossible regardless of intrinsic motivation. [Note: prior citations F-001 "H4 Severity 3 stale skills table" was INVALIDATED by live-site rescope — the live skills table is current; F-020 is the current live-site equivalent at Severity 2. F-004b "Guides section references only 5 entries" is superseded by F-013 in the live-site rescope.] |
...
| **GSM Derivation** | B=MAP (FEAT-040-006): Target behavior is "After landing on README.md, successfully invoke first Jerry skill with JERRY_PROJECT set and produce a persistent artifact within 15 minutes." Bottleneck = Multiple (Prompt + Ability). Step 3 missing Facilitator (Prompt-primary). Cumulative Brain Cycles=2 from developer-novel elements (Ability-primary). F-014 (Severity 3, FEAT-040-004 rescope-iter-2): sidebar navigation lacks breadcrumbs and recognition affordances — users cannot locate where they are in the setup flow. F-016 (Severity 2, FEAT-040-004 rescope-iter-2): prerequisites checklist not surfaced before Quick Start — users discover missing prerequisites mid-flow. These combine to create a measurable adoption barrier with a specific intervention opportunity (Step 3 restructure + prerequisites checklist, B=MAP Intervention #1). [Note: prior citation F-010 "branching instructions hidden from upfront view, Severity 3" from degraded-mode FEAT-040-004 iter-4 is superseded; the live-site equivalents are F-014 and F-016.] |
...
| **GSM Derivation** | F-013 (H10, Severity 3, FEAT-040-004 rescope-iter-2): skills table lacks hyperlinks to playbooks or guides — users who want to return for a second skill have no navigation path from the entry point to that skill's documentation. Diataxis audit: zero tutorial coverage for any of 30 skills; 16 skills added since baseline have zero documentation whatsoever. B=MAP Behavior Change Roadmap (FEAT-040-006): "INSTALLATION.md B=MAP as independent behavioral surface" — retention analysis cannot begin until adoption is confirmed. Retention is downstream of Adoption; low-confidence in Phase 1a. [Note: prior citation F-004b "23 of 30 skills not referenced in the Guides section, Severity 3" is superseded; F-013 is the current live-site equivalent capturing the navigation linkage gap.] |
...
| **GSM Derivation** | F-014 (H6, Severity 3, FEAT-040-004 rescope-iter-2): sidebar navigation lacks breadcrumbs and "You Are Here" affordances — users cannot determine where they are in the setup flow and must recall section structure rather than recognize it visually. F-016 (H5, Severity 2, FEAT-040-004 rescope-iter-2): prerequisites checklist not surfaced before Quick Start — users encounter missing prerequisites mid-task and must restart. B=MAP (FEAT-040-006): Step 3 Prompt failure is "directly observable — routing info embedded mid-step. Structural not inferential." W-001 (SC 1.3.1, Severity 3, FEAT-040-005 rescope-iter-5): INSTALLATION.md bold-text step labels violate heading hierarchy — AT users cannot navigate steps programmatically, creating task success failure for screen reader users. [Note: prior citation F-010 "branching instructions hidden in Step 3, Severity 3" from degraded-mode FEAT-040-004 iter-4 is superseded; the live-site equivalents are F-014 and F-016. W-002 "non-descriptive link text 'file it'" was REMOVED as a false positive in FEAT-040-005 rescope-iter-2 — "file it" is plain prose, not a hyperlink — and is not cited.] |
...
| Skill Discovery Rate | Engagement | (Users who visit documentation for > 7 distinct skills / Total active users) × 100 — **Threshold calibration note:** The "7" threshold is a Phase 1a approximation of 23% of the 30-skill catalog (7/30 = 0.233); recalibrate proportionally after F-020 remediation expands entry-point skill visibility. | Page analytics — documentation page views with skill slug extracted from URL path | Weekly | >= 25% | < 10% for 2 consecutive weeks | TBD |
...
```
*Evidence for Model A:* B=MAP bottleneck at Step 3 (FEAT-040-006), F-014 Severity 3 + F-016 Severity 2 (FEAT-040-004 rescope-iter-2) — structural task failure at the adoption entry point. If the barrier is mechanical (missing navigation recognition, missing prerequisites before Quick Start), fixing Task Success should unlock adoption.
...
2. **Judgment: Getting-Started Completion Rate is the single highest-impact metric.** This claim rests on three converging signals (B=MAP Major bottleneck, F-014 Severity 3 + F-016 Severity 2 per FEAT-040-004 rescope-iter-2, F-020 Severity 2 per FEAT-040-004 rescope-iter-2) that all trace to the getting-started and discovery flow. No alternative metric from the upstream analysis has equivalent triple-convergence support. [Note: prior citations F-010 and F-001 from degraded-mode FEAT-040-004 iter-4 are superseded by F-014/F-016 and F-020 respectively; the underlying evidential basis is unchanged.]
...
|--------------------|------------------------|----------------------|--------------|-----------------|
| First-time adopter | Adoption + Task Success | Blocked at Step 3 branch decision; goal is first-skill invocation | B=MAP bottleneck (FEAT-040-006), F-014 Severity 3 + F-016 Severity 2 (FEAT-040-004 rescope-iter-2) | Task Success failures also affect experienced users attempting new workflows — this segment may be too narrow |
| Skill explorer | Engagement + Retention | Past first-skill invocation; expanding skill use; blocked by invisible skill catalog | F-020 Severity 2 (FEAT-040-004 rescope-iter-2), F-013 Severity 3 (FEAT-040-004 rescope-iter-2) | Retention dimension lacks a dedicated segment; returning-user needs may warrant a separate archetype |
| Trust evaluator | Happiness | Evaluating framework investment; sensitive to social proof; blocked by inconsistent messaging | F-011 Severity 3 (FEAT-040-004 rescope-iter-2), FM-001 Belonging=3 (FEAT-040-006) | May overlap with first-time adopter; causal relationship with adoption is an open question (see Metric Interdependencies) |
...
| Skill Discovery Rate | Engagement | (Users > 7 skills / Total users) × 100 — "7" = Phase 1a approximation of 23% of 30-skill catalog (7/30 = 0.233); recalibrate proportionally post-F-020-remediation | >= 25% | LOW | Analytics not instrumented |
...
| INC-001-resolution | 2026-04-20 | 0.921-0.924 (expected) | CITATION UPDATE | Surgical citation update per QG-2 INC-001 finding. All stale/invalidated finding IDs from degraded-mode FEAT-040-004 iter-4 replaced with current live-site IDs from FEAT-040-004 rescope-iter-2. Changes: (1) F-001 → F-020 in Executive Summary, HEART Dimension Selection, Engagement GSM, Metric Specifications table, Handoff Data table; (2) F-003 + F-007 → F-011 in Executive Summary, Happiness dimension selection, Happiness GSM derivation, Handoff Data table; (3) F-010 → F-014 + F-016 in Executive Summary, Task Success dimension selection, Adoption GSM derivation, Task Success GSM derivation, Model A evidence, Synthesis Judgment 2, Handoff Data table; (4) F-004b → F-013 in Retention dimension selection, Retention GSM derivation, Handoff Data table; (5) W-002 removed from Task Success dimension selection row and Task Success GSM derivation (W-002 confirmed REMOVED as false positive in FEAT-040-005 rescope-iter-2). No structural changes to dimension selections, metric specifications, thresholds, or handoff XP data. All citations now reference FEAT-040-004 rescope-iter-2 as the authoritative source. Traceability notes added inline at each change site. |
...
*Agent: ux-heart-analyst v1.0.0 | FEAT-040-002 Phase 1a Provisional | 2026-04-20 | Iter-5 + INC-001-resolution (citation update 2026-04-20)*
