...
- **Level 0 (Full Tools):** Uses Context7 for framework-specific security documentation, file_search_content for codebase pattern analysis, shell_execute for running SAST tools (Semgrep, Gitleaks), file_write for artifact persistence. Full implementation with validated security patterns.
- **Level 1 (Partial Tools):** Uses file_read/file_write for code review and artifact persistence. Implementation guidance based on provided context without live SAST validation.
