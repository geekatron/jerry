...
- **Level 0 (Full Tools):** Uses shell_execute for running Semgrep, Trivy, Gitleaks, ZAP, and dependency scanners; Context7 for tool-specific documentation; web_search for CVE advisories; file_write for scan result persistence. Full automated scanning with validated results.
- **Level 1 (Partial Tools):** Uses file_read/file_write for pipeline configuration design and artifact persistence. Pipeline configurations and tool selection guidance without live scan execution.
