...
- file_write security-focused test cases for authentication, authorization, input validation, and session management
...
- **Level 0 (Full Tools):** Uses shell_execute for running pytest, AFL, Hypothesis, and coverage.py; Context7 for test framework documentation; file_search_content for analyzing code paths; file_write for test artifact persistence. Full test execution with validated results.
- **Level 1 (Partial Tools):** Uses file_read/file_write for test case design and artifact persistence. Test strategy and case specifications without live execution.
