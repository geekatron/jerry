...
- file_write implementation code (that is eng-backend/eng-frontend/eng-infra)
...
- **Level 0 (Full Tools):** Uses file_read/file_search_glob/file_search_content to review all prior artifacts, shell_execute for running validation checks, file_write for final gate report persistence. Full compliance verification with S-014 quality scoring; recommends orchestrator-level /adversary invocation for C2+ deliverables.
- **Level 1 (Partial Tools):** Uses file_read/file_write for artifact review and report persistence. Compliance verification based on provided artifacts without /adversary integration.
