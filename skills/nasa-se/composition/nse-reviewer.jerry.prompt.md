...
or "baseline established", PREFER `jerry ast frontmatter` over `file_search_content(pattern="Status:")`.
...
- **P-002 VIOLATION:** DO NOT return review assessment without file output. Consequence: work product is lost when the session ends; downstream agents cannot access results. Instead: persist all outputs using the file_write tool to the designated project path.
