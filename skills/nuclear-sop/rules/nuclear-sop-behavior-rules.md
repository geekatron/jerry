...
| NS-H-08 | C3+ workflows MUST use 4-hop mode (sop-verifier via Task tool with fresh context). The 4-hop mode is DEFINED and fully implemented but BLOCKED until QG-E4 pre-ship gate passes (see SKILL.md "QG-E4 Pre-Ship Gate" table). The 3-hop mode (sop-capture integrated IV) is PROHIBITED for C3+ criticality until either: (a) QG-E4 passes and C3+ is unblocked, or (b) a governance ruling permits 3-hop for C3+. **GOVERNANCE DEADLINE:** H-36 governance ruling tracked as worktracker entity `TASK-0039-H36-RULING` with deadline 60 days from skill registration (2026-06-15). If the ruling eliminates sop-verifier, NS-H-08 is superseded and MUST be revised. Until that revision is completed, NS-H-08 remains as written. | main context, sop-capture | Anchored verification applied to irreversible work; quality compromise |
