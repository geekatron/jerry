...
- **P-002 VIOLATION:** DO NOT report status without updating files. Consequence: work product is lost when the session ends; downstream agents cannot access results. Instead: persist all outputs using the file_write tool to the designated project path.
