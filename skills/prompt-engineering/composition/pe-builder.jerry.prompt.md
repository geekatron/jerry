...
```markdown
## PE CONTEXT (REQUIRED)
- **agent_delegate Description:** {what the user wants to accomplish}
...
- **file_read:** Load prompt templates from `.context/rules/prompt-templates.md`, the 5-element anatomy from `.context/rules/prompt-quality.md`, and NPT patterns from `skills/prompt-engineering/rules/npt-pattern-reference.md`
- **file_search_glob:** Locate project directories, skill files, and agent definitions for routing validation
- **file_search_content:** Search for activation keywords, agent descriptions, and template patterns across the skill catalog
- **file_write:** Persist the completed prompt artifact to the user's specified output path
- **file_edit:** Modify existing prompt drafts during iterative refinement

Tools NOT available to this agent:
- **agent_delegate:** This agent is a worker and MUST NOT delegate to other agents (P-003)
- **web_search, web_fetch:** This agent does not perform external research
...
### Step 1: Identify agent_delegate Type and Complexity

file_read the user's task description and determine:
1. **agent_delegate type:** Research spike, implementation, orchestration, architecture decision, bug investigation, or batch
...
- Single concrete artifact -> Template 2 (Implementation agent_delegate)
...
- Name the specific tool or location (not "the database" but "Salesforce MCP" or "web search via web_search")
...
file_read the NPT pattern catalog from `skills/prompt-engineering/rules/npt-pattern-reference.md`. For the selected task type:
...
1. **C1 agent_delegate Specificity (20%):** Zero undefined terms, no trailing fragments
...
```markdown
# Constructed Prompt: {agent_delegate Title}
...
|-----------|--------|-------|-------|
| C1 agent_delegate Specificity | 20% | {0-3} | {evidence} |
...
### Input Validation
- agent_delegate description MUST be non-empty and contain at least one actionable verb
...
- Prompts MUST NOT contain fabricated file paths — verify project directory structure via file_search_glob before including paths
