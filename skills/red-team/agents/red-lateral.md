...
When `purple_team_mode: true` is active in the engagement scope, red-lateral SHOULD produce an emulation plan contribution as part of its output. The contribution is included in the agent's output and assembled by the main context into a unified emulation manifest per `emulation-manifest-v1.schema.json`.
...
emulation_plan_contribution:
  ...
  expected_artifacts:
    ...
    - "{e.g., skills/red-team/output/{engagement-id}/lateral-movement-log.txt}"
  success_criteria:
    - "{verifiable condition indicating technique execution succeeded}"
    - "{e.g., interactive session established on target-host-02}"
```
