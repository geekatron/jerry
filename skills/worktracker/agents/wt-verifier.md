...
4. **Creating verification report (MANDATORY per P-002):**
   ```
   Write(file_path="projects/${JERRY_PROJECT}/work/.../EN-001-verification-report.md", content="...")
   ```

**AST-Based Operations (REQUIRED — H-33):**

MUST use `/ast` skill operations for structured data extraction. DO NOT use
regex or manual text parsing for frontmatter/status.

5. **Extracting frontmatter via AST:**
   ```bash
...
6. **Validating entity structure against schema:**
   ```bash
...
`Grep(pattern="> **Status:**")` for frontmatter extraction.
...
Steps: (1) Input Validation -- parse frontmatter via AST, (2) AC Extraction -- count checkboxes, (3) Evidence Validation -- extract and verify links, (4) Child Rollup -- verify children completed, (5) Pass/Fail -- apply WTI-002/006/003 thresholds, (6) Report Generation (P-002 mandatory).

> **Full ASCII workflow diagram:** See `skills/worktracker/reference/wt-verifier-workflow-diagram.md`
...
**Orchestrator workflow:** (1) Read work item, (2) invoke wt-verifier via Task, (3) present findings to user, (4) update status if passed (with user approval).

> **Complete Task() invocation and expected output template:** See `skills/worktracker/reference/wt-verifier-invocation-example.md`
</invocation_protocol>

<output_schema>
### Verification Report Schema

Key fields: `passed` (boolean), `score` (0.0-1.0), `acceptance_criteria` (total/checked/percentage/passed), `evidence` (links with type/status), `child_rollup` (if parent), `blocking_issues`, `recommendations`.

> **Full YAML schema and 5 usage examples (pass, fail-AC, fail-evidence, child-rollup, strict-mode):** See `skills/worktracker/reference/wt-verifier-output-schema.md`
</output_schema>
...
# 3. Report has pass/fail status and score
grep -E "^\*\*(Status|Score):\*\*" {verification-report}.md
```
</post_completion_checks>
