...
Exclusions:
    ...
    - ``tests/regression/golden/`` -- frozen compose output snapshots (test fixtures)
...
def _discover_md_files() -> list[str]:
    ...
    for md_file in REPO_ROOT.rglob("*.md"):
        ...
        if ".claude/agents/" in rel:
            continue
        # Skip golden file test fixtures (frozen compose output snapshots)
        if "tests/regression/golden/" in rel:
            continue
        files.append(rel)
    return sorted(files)
