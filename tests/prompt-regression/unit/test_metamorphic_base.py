...
class TestMRViolationSeverity:
    """Verify MRViolationSeverity enum members (FR-010)."""

    def test_pass_member_should_have_correct_value(self) -> None:
        """PASS must be present with value 'PASS'."""
        assert MRViolationSeverity.PASS.value == "PASS"

    def test_informational_member_should_have_correct_value(self) -> None:
        """INFORMATIONAL must be present."""
        assert MRViolationSeverity.INFORMATIONAL.value == "INFORMATIONAL"

    def test_warning_member_should_have_correct_value(self) -> None:
    ...
    def test_critical_member_should_have_correct_value(self) -> None:
...
class TestMRResult:
    """MRResult is a frozen dataclass (FR-010)."""
    ...
    def test_mr_result_should_be_frozen(self) -> None:
        """MRResult should be immutable (frozen=True)."""
        result = self._make_result()
        with pytest.raises((dataclasses.FrozenInstanceError, AttributeError)):
            result.passed = False  # type: ignore[misc]

    def test_mr_result_should_expose_all_required_fields(self) -> None:
    ...
    def test_mr_result_should_have_correct_mean_delta(self) -> None:
...
class TestValidateInputsInsufficientSamples:
    """_validate_inputs() raises InsufficientSamplesError when N < minimum_sample_size (FR-010)."""

    def test_insufficient_samples_should_raise(self, stub_mr: _ConcreteStubMR) -> None:
    ...
    def test_exactly_minimum_should_not_raise(self, stub_mr: _ConcreteStubMR) -> None:
        """N == 20 (minimum_sample_size) should not raise."""
        scores = _valid_scores(20)
        # Should not raise
        stub_mr._validate_inputs(scores, scores)

    def test_insufficient_error_message_should_include_mr_id_and_minimum(
        self, stub_mr: _ConcreteStubMR
    ) -> None:
...
class TestValidateInputsLengthMismatch:
    """_validate_inputs() raises ValueError when sequence lengths differ (FR-010)."""

    def test_length_mismatch_should_raise(self, stub_mr: _ConcreteStubMR) -> None:
    ...
    def test_equal_lengths_should_not_raise(self, stub_mr: _ConcreteStubMR) -> None:
...
class TestValidateInputsOutOfRange:
    """_validate_inputs() raises ValueError for scores outside [0.0, 1.0] (FR-010)."""

    def test_score_above_one_should_raise(self, stub_mr: _ConcreteStubMR) -> None:
    ...
    def test_score_below_zero_should_raise(self, stub_mr: _ConcreteStubMR) -> None:
    ...
    def test_boundary_values_should_be_accepted(self, stub_mr: _ConcreteStubMR) -> None:
...
class TestStatisticalHelpers:
    """Test the shared statistical helper methods (FR-010)."""

    def test_mean_helper_should_return_correct_arithmetic_mean(
        self, stub_mr: _ConcreteStubMR
    ) -> None:
    ...
    def test_std_helper_should_return_zero_for_single_value(self, stub_mr: _ConcreteStubMR) -> None:
        """_std() with a single value should return 0.0."""
        assert stub_mr._std([0.93]) == 0.0

    def test_std_helper_should_return_nonnegative_for_multiple_values(
        self, stub_mr: _ConcreteStubMR
    ) -> None:
...
class TestParaphraseConsistency:
    """Tests for MR-001 ParaphraseConsistency transform and evaluate (FR-010)."""
    ...
    def test_transform_should_return_non_empty_string(self, mr: ParaphraseConsistency) -> None:
        """transform() should return a non-empty string."""
        result = mr.transform("Research the topic carefully.")
        assert isinstance(result, str)
        assert len(result) > 0

    def test_transform_should_raise_for_empty_input(self, mr: ParaphraseConsistency) -> None:
        """transform() with empty input should raise ValueError."""
        with pytest.raises(ValueError):
            mr.transform("")

    def test_transform_should_raise_for_whitespace_only_input(
        self, mr: ParaphraseConsistency
    ) -> None:
        """transform() with whitespace-only input should raise ValueError."""
        with pytest.raises(ValueError):
            mr.transform("   ")

    def test_transform_should_apply_known_substitution(self, mr: ParaphraseConsistency) -> None:
    ...
    def test_transform_should_be_deterministic(self, mr: ParaphraseConsistency) -> None:
    ...
    def test_evaluate_should_raise_for_insufficient_samples(
        self, mr: ParaphraseConsistency
    ) -> None:
    ...
    def test_mr_id_and_name_should_match_specification(self, mr: ParaphraseConsistency) -> None:
        """MR-001 should have correct mr_id and mr_name."""
        assert mr.mr_id == "MR-001"
        assert mr.mr_name == "Paraphrase Consistency"

    def test_minimum_sample_size_should_equal_20(self, mr: ParaphraseConsistency) -> None:
...
class TestNegationHandling:
    """Tests for MR-002 NegationHandling transform and evaluate (FR-010)."""
    ...
    def test_transform_should_return_non_empty_string(self, mr: NegationHandling) -> None:
        """transform() should return a non-empty string."""
        result = mr.transform("The agent must include L0/L1/L2 sections.")
        assert isinstance(result, str)
        assert len(result) > 0

    def test_transform_should_raise_for_empty_input(self, mr: NegationHandling) -> None:
        """transform() with empty input should raise ValueError."""
        with pytest.raises(ValueError):
            mr.transform("")

    def test_transform_should_apply_known_negation_pattern(self, mr: NegationHandling) -> None:
        """'must include' should be transformed to 'must NOT include'."""
        result = mr.transform("The output must include citations.")
        assert "NOT" in result or "not" in result

    def test_transform_should_add_fallback_prefix_for_unmatched_pattern(
        self, mr: NegationHandling
    ) -> None:
    ...
    def test_evaluate_should_raise_for_insufficient_samples(self, mr: NegationHandling) -> None:
        """evaluate() with N < 15 should raise InsufficientSamplesError."""
        small = [0.93] * 10
        with pytest.raises(InsufficientSamplesError):
            mr.evaluate(small, small)

    def test_mr_id_and_name_should_match_specification(self, mr: NegationHandling) -> None:
        """MR-002 should have correct mr_id and mr_name."""
        assert mr.mr_id == "MR-002"
        assert mr.mr_name == "Negation Handling"

    def test_minimum_sample_size_should_be_reduced_to_15(self, mr: NegationHandling) -> None:
