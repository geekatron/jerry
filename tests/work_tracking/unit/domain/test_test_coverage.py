...
from src.work_tracking.domain.value_objects.test_coverage import TestCoverage


class TestTestCoverageCreation:
    """Tests for TestCoverage initialization."""

    def test_create_from_percent(self) -> None:
        """TestCoverage can be created from percentage value."""
    ...
    def test_from_percent_rounds_to_two_decimals(self) -> None:
        ...
        coverage = TestCoverage.from_percent(85.567)
        assert coverage.percent == 85.57

    def test_create_from_fraction(self) -> None:
        """TestCoverage can be created from covered/total fraction."""
        coverage = TestCoverage.from_fraction(85, 100)
    ...
    def test_from_fraction_calculates_percentage(self) -> None:
        ...
        coverage = TestCoverage.from_fraction(7, 10)
        assert coverage.percent == 70.0

    def test_from_fraction_with_decimals(self) -> None:
        """from_fraction rounds to 2 decimal places."""
        coverage = TestCoverage.from_fraction(1, 3)
        assert coverage.percent == 33.33

    def test_direct_construction(self) -> None:
        """TestCoverage can be constructed directly."""
        coverage = TestCoverage(percent=50.0)
        assert coverage.percent == 50.0


class TestTestCoverageThreshold:
    ...
    def test_meets_threshold_when_equal(self) -> None:
        ...
        coverage = TestCoverage.from_percent(80.0)
        assert coverage.meets_threshold(80.0) is True
    ...
    def test_gap_to_threshold_when_above(self) -> None:
        ...
        coverage = TestCoverage.from_percent(95.0)
    ...
    def test_gap_to_threshold_when_equal(self) -> None:
        ...
        coverage = TestCoverage.from_percent(90.0)
        assert coverage.gap_to_threshold(90.0) == 0.0


class TestTestCoverageComparison:
    """Tests for comparison operations."""

    def test_less_than(self) -> None:
        """TestCoverage supports < comparison."""
        low = TestCoverage.from_percent(50.0)
        high = TestCoverage.from_percent(80.0)
        assert low < high
        assert not high < low

    def test_less_than_or_equal(self) -> None:
        """TestCoverage supports <= comparison."""
        a = TestCoverage.from_percent(50.0)
        b = TestCoverage.from_percent(50.0)
        c = TestCoverage.from_percent(80.0)
        assert a <= b
        assert a <= c
        assert not c <= a
    ...
    def test_equality(self) -> None:
        """TestCoverage supports equality comparison."""
        a = TestCoverage.from_percent(75.0)
        b = TestCoverage.from_percent(75.0)
        assert a == b

    def test_inequality(self) -> None:
        """TestCoverage supports inequality comparison."""
        a = TestCoverage.from_percent(75.0)
        b = TestCoverage.from_percent(80.0)
        assert a != b

    def test_comparison_with_non_coverage_returns_not_implemented(self) -> None:
        """Comparison with non-TestCoverage returns NotImplemented."""
        coverage = TestCoverage.from_percent(50.0)
...
class TestTestCoverageStringRepresentation:
