...
from src.work_tracking.domain.value_objects.type_ratio import QualityLevel, TypeRatio
...
class TestTypeRatioCreation:
    """Tests for TypeRatio initialization."""

    def test_create_with_all_types(self) -> None:
        """TypeRatio can be created with all test types."""
    ...
    def test_create_positive_only(self) -> None:
        """TypeRatio can be created with only positive tests."""
        ratio = TypeRatio(positive=10, negative=0, edge_case=0)
        assert ratio.positive == 10

    def test_create_balanced(self) -> None:
        """TypeRatio can be created with balanced distribution."""
        ratio = TypeRatio(positive=10, negative=10, edge_case=10)
        assert ratio.total == 30


class TestTypeRatioProperties:
    """Tests for TypeRatio computed properties."""
...
class TestTypeRatioQualityLevels:
    ...
    def test_fails_l1_without_negative(self) -> None:
        ...
        ratio = TypeRatio(positive=5, negative=0, edge_case=2)
...
class TestTypeRatioHighestLevel:
...
class TestTypeRatioStringRepresentation:
    ...
    def test_repr_format(self) -> None:
        ...
        ratio = TypeRatio(positive=5, negative=3, edge_case=2)
        expected = "TypeRatio(positive=5, negative=3, edge_case=2)"
        assert repr(ratio) == expected


class TestTypeRatioEdgeCases:
    """Edge case tests for TypeRatio."""
    ...
    def test_large_counts(self) -> None:
        ...
        ratio = TypeRatio(positive=1000, negative=500, edge_case=250)
        assert ratio.total == 1750

    def test_immutability(self) -> None:
        """TypeRatio is immutable (frozen dataclass)."""
        ratio = TypeRatio(positive=5, negative=3, edge_case=2)
        with pytest.raises(FrozenInstanceError):
            ratio.positive = 10  # type: ignore

    def test_hashable(self) -> None:
        """TypeRatio can be used as dict key."""
        r1 = TypeRatio(positive=5, negative=3, edge_case=2)
        r2 = TypeRatio(positive=5, negative=3, edge_case=2)
        ratio_map = {r1: "good"}
        assert ratio_map[r2] == "good"

    def test_equality(self) -> None:
        """TypeRatios with same values are equal."""
        r1 = TypeRatio(positive=5, negative=3, edge_case=2)
        r2 = TypeRatio(positive=5, negative=3, edge_case=2)
        assert r1 == r2

    def test_inequality(self) -> None:
        """TypeRatios with different values are not equal."""
        r1 = TypeRatio(positive=5, negative=3, edge_case=2)
        r2 = TypeRatio(positive=5, negative=4, edge_case=2)
        assert r1 != r2


class TestTypeRatioValidation:
    """Negative tests for TypeRatio validation."""

    def test_all_zeros_rejected(self) -> None:
        """At least one test is required."""
        with pytest.raises(ValueError, match="At least one test"):
            TypeRatio(positive=0, negative=0, edge_case=0)

    def test_negative_positive_rejected(self) -> None:
        """Negative positive count is rejected."""
        with pytest.raises(ValueError, match="non-negative"):
            TypeRatio(positive=-1, negative=3, edge_case=2)

    def test_negative_negative_rejected(self) -> None:
        """Negative negative count is rejected."""
        with pytest.raises(ValueError, match="non-negative"):
            TypeRatio(positive=5, negative=-1, edge_case=2)

    def test_negative_edge_case_rejected(self) -> None:
        """Negative edge_case count is rejected."""
        with pytest.raises(ValueError, match="non-negative"):
            TypeRatio(positive=5, negative=3, edge_case=-1)

    def test_non_integer_positive_rejected(self) -> None:
        """Non-integer positive count is rejected."""
        with pytest.raises(TypeError, match="must be an integer"):
            TypeRatio(positive=5.5, negative=3, edge_case=2)  # type: ignore

    def test_non_integer_negative_rejected(self) -> None:
        """Non-integer negative count is rejected."""
        with pytest.raises(TypeError, match="must be an integer"):
            TypeRatio(positive=5, negative=3.5, edge_case=2)  # type: ignore

    def test_non_integer_edge_case_rejected(self) -> None:
        """Non-integer edge_case count is rejected."""
        with pytest.raises(TypeError, match="must be an integer"):
            TypeRatio(positive=5, negative=3, edge_case=2.5)  # type: ignore

    def test_string_count_rejected(self) -> None:
        """String count is rejected."""
        with pytest.raises(TypeError, match="must be an integer"):
            TypeRatio(positive="5", negative=3, edge_case=2)  # type: ignore
